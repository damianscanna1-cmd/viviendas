...
# -----------------------------------------------------------------------------
# CONFIGURACIÓN Y BLOQUEO CSS (PARA ELIMINAR ELEMENTOS FLOTANTES)
# -----------------------------------------------------------------------------
st.set_page_config(page_title="Dossier GHS", layout="wide")

st.markdown("""
    <style>
    /* Ocultamiento absoluto de elementos nativos de la plataforma */
...
# -----------------------------------------------------------------------------
# FUNCIONES DE DATOS Y AUXILIARES
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# NAVEGACIÓN Y LÓGICA DE ACCESO
# -----------------------------------------------------------------------------
st.title("🔒 Dossier Inmobiliario GHS")

modo = st.radio("Seleccione su tipo de acceso:", ["Cliente", "Administrador"])
...
if modo == "Cliente":
    ...
    if prop_keys:
        prop_sel = st.selectbox("Selecciona la Propiedad", prop_keys)
        prop_data = db["propiedades"][prop_sel]
        
        clave = st.text_input("Introduce la contraseña para ver la propiedad:", type="password")

        if clave == prop_data["password_cliente"]:
            st.success("Acceso autorizado")
            st.markdown("---")

            st.header(prop_data["titulo_es"])
            st.caption(f"📍 {prop_data['ubicacion']}")

            # Galería de Fotografías
            imagenes = prop_data.get("imagenes", [])
            if imagenes:
                st.subheader("📸 Galería de Fotografías")
                render_galeria(imagenes, is_es=True)
            else:
                st.info("No hay fotos subidas para esta propiedad.")
            ...
            col1.metric("Superficie", prop_data["superficie"])
            col2.metric("Habitaciones", prop_data["habitaciones"])
            col3.metric("Baños", prop_data["banos"])
            col4.metric("Precio", prop_data["precio"])

            st.subheader("Descripción")
            st.write(prop_data["descripcion_es"])

            st.markdown("---")
            st.link_button("💬 Contactar por WhatsApp", f"https://wa.me/{WHATSAPP_NUMBER}")
